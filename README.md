...
* **Python Version**: `3.12.3` (Developed and tested with this version).
* **Dependencies**: This project uses Python standard libraries only; no third-party `pip` packages are required.
    * `tkinter`: For the GUI (usually included with Python).
    * `math`, `struct`, `os`, `sys`, `subprocess`, `threading`: Core standard libraries.
...
This tool strictly adheres to the **SMPTE ST 2095-1:2023** standard. Pink noise is a random signal with equal energy per octave (power spectral density is inversely proportional to the frequency, $1/f$).
...
4.  **Level Calibration**:
    The algorithm automatically scales the gain to align the output signal's RMS level precisely to the standard's target of **-18.5 dBFS(AES17)** (equivalent to **-21.5 dBFS RMS**).
